...
import { profileValid } from '../../Utils/ProfileValid';

const Profile = ({ profile, user: { loading }, handleUpdateProfile, handleUploadImage, handleClick, handleOnSubmit }) => {
    // Reg Exp
    ...
    const validation = Yup.object().shape({
        ...
        confirmPass: Yup.string().oneOf([Yup.ref('password')], "Password is not match"),
        name: Yup.string(),
                                ...
                                {({ values, errors, handleChange, handleSubmit }) =>
                                    <form onSubmit={handleSubmit} >
                                        ...
                                        <div className="change-pass detail">
                                            ...
                                            <div className="row">
                                                <div className="col-md-12 col-lg-6 col-xl-6">
                                                    <Input clName="form-group full-name" labelName="Current Password" name="oldPass" type="password" value={values.oldPass} onChange={handleChange} />
                                                    {errors.oldPass && <p className="text-danger">{errors.oldPass}</p>}
                                                </div>
                                            </div>
                                            <div className="row">
                                                <div className="col-md-12 col-lg-6  col-xl-6">
                                                    <Input clName="form-group w-100" labelName="New Password" name="newPass" type="password" value={values.newPass} onChange={handleChange} />
                                                    {errors.newPass && <p className="text-danger">{errors.newPass}</p>}
                                                </div>
                                                <div className="col-md-12 col-lg-6 col-xl-6">
                                                    <Input clName="form-group w-100" labelName="Confirm Password" name="confirmPass" type="password" value={values.confirmPass} onChange={handleChange} />
                                                    {errors.confirmPassword && <p className="text-danger">{errors.confirmPass}</p>}
                                                </div>
                                            </div>
                                        </div>
